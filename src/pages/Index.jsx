    ...
    <Grid templateColumns="repeat(8, 1fr)" gap={0} width="400px" margin="auto">
      {board.map((row, rowIndex) =>
        row.map((cell, colIndex) => (
          <GridItem key={`${rowIndex}-${colIndex}`} bg={(rowIndex + colIndex) % 2 === 0 ? "gray.100" : "gray.400"} height="50px" display="flex" alignItems="center" justifyContent="center" fontSize="2xl" color={cell[0] === "w" ? "white" : "black"}>
            {getPieceIcon(cell)}
          </GridItem>
        )),
      )}
